...
static NSString *format(NSString *string, NSString *country){
    ...
    for(int i = 0, j = 0; i < format.length && j < string.length; i++){
        NSString *currentFormatChar = [format substringWithRange:NSMakeRange(i, 1)];
        NSString *currentNumberChar = [string substringWithRange:NSMakeRange(j, 1)];
        if([currentFormatChar isEqualToString:@"x"]){
            [formattedNumber appendString:currentNumberChar];
            j++;
        } else {
            [formattedNumber appendString:currentFormatChar];
        }
    }
    
    return [formattedNumber copy];
}
...
- (NSDictionary*)converToPhoneNumberNextString:(NSString*)string; {
    ...
    NSString *formattedString = trimPlus(string);
    NSDictionary *countryInfo = getCountryInfo(formattedString);
    NSString *countryCode = countryInfo[KeyCountryCode];
    NSString *country = countryInfo[KeyCountry];
    if(country.length == 0) {
        if(formattedString.length > MaxNumberLength){
            formattedString = [formattedString substringWithRange:NSMakeRange(0, MaxNumberLength)];
        }
        result[KeyPhoneNumber] = [NSString stringWithFormat:@"+%@", formattedString];
    } else {
        formattedString = [string substringFromIndex:countryCode.length];
        formattedString = format(formattedString, country);
